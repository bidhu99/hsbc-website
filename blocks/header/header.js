...
function extractHeaderData() {
    ...
    const firstLevelDivs = Array.from(parent.children).filter(child => child.tagName === "DIV");
    ...
    firstLevelDivs.forEach((block, index) => {
        ...
        const childNodeArray = Array.from(block.children).filter(child => child.tagName === "DIV")
...
function readDataFromParagraphTag(childBlock){
  ...
  const htmlContent = (pTag ? pTag.innerHTML : childBlock.innerHTML)?.trim() || "";
  return htmlContent;
}
...
function buildHeader(data) {
    ...
    data.sections.forEach((section, index) => {
        ...
        titleDiv.className = `header-mobile-doormat-${index} sidebar-submenu-trigger`;
