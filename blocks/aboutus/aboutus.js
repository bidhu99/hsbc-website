...
export default function decorate(block) {
  ...
  const headings = Array.from(blocks[0].querySelectorAll('p')).map(p => p.textContent.trim());
...
function createMasterCards(data) {
  ...
  grid.className = 'crh-master-card__grid crh-master-card__grid-off crh-master-card__light-pearl';
  ...
  data.forEach((section, index) => {
    ...
    card.className = `M-MASTERCARD crh-card crh-master-cards__card crh-master-cards__card--hide-frame${index === 0 ? ' crh-master-cards__first-card' : index === 1 ? ' crh-master-cards__margin-2' : ''}`;
    ...
    section.items.forEach((item, idx) => {
      ...
      anchor.className = 'A-LNKC16R-RW-ALL A-TYPS5R-RW-DEV master-card-chevron-link';
      anchor.href = item.link;
      anchor.target = '_self';
      anchor.setAttribute('data-event-component', 'text link');
      anchor.setAttribute('data-event-name', `${item.title.toLowerCase()}|component:${section.heading.toLowerCase()}|position:${index + 1}`);
