...
function buildAccordion(section, index) {
  ...
  const accordionRow = createEl("div", [
    "O-SMARTSPCGEN-DEV",
    "O-ADVEXP-RW-RBWM",
    "row",
  ]);
  ...
  section.content.forEach((item) => {
    ...
    if (item.type === "text" && item.value) {
      ...
      const linksWrapper = createEl("div", [
        "O-SMARTSPCGEN-DEV",
        "M-CONTMAST-RW-RBWM",
        "links",
      ]);
...
function buildMarkup(data, container) {
  ...
  const rowWrapper = createEl("div", [
    "O-SMARTSPCGEN-DEV",
    "O-ACCRD-RW-RBWM",
    "row",
  ]);
  const div = document.createElement("div");
  const accordionsContainer = createEl("div", [
  ...
  data.forEach((section, idx) => {
    ...
    if (accordion) accordionsContainer.appendChild(accordion);
    // if (accordion) leftCol.appendChild(accordion);
  });

  div.appendChild(accordionsContainer);
  rowWrapper.appendChild(div);
  leftCol.appendChild(rowWrapper);
